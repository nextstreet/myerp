...
No database migration is added in v0.5.
...
The response should contain `version: "0.5.0"` and `consoleConfigured: true`.
...
1. **Products** — filter products and open the review page.
...
3. **Review** — edit variants, upload images, assign one primary image per color, discover site categories, save English/Spanish content and calculate/store independent site prices.
4. **Publish preflight** — run local or authenticated read-only preflight, inspect missing fields and the Global UP candidate, and view redacted job logs.
