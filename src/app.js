...
export async function buildApp({ config, pool, mercadoLibreClient }) {
  const app = Fastify({
    logger: {
  ...
  app.decorate('mercadoLibre', mercadoLibreClient);

  app.get('/health', async () => {
    await pool.query('SELECT 1');
    return { ok: true, service: 'mercado-libre-ai-listing-console', version: '0.1.0' };
  });
